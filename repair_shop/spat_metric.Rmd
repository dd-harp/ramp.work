...
author: "Sean L Wu and David L Smith"
...
VC_matrix <- make_VC_matrix(f = f, q = q, Omega = Omega, tau = tau, M = M, W = W)
HTC_matrix <- make_HTC_matrix(W = W, beta = beta, b = b, D = D, H = H)
calR <- make_calR(b = b, beta = beta, VC_matrix = VC_matrix, W = W, D = D, H = H)
calZ <- make_calZ(Omega = Omega, tau = tau, f = f, q = q, M = M, W = W, HTC_matrix = HTC_matrix)
```

We can visualize the matrices as heatmaps.

```{r, out.width = "100%"}
VC_matrix_df <- data.frame(
  value = as.vector(VC_matrix),
  x = rep(1:nrow(VC_matrix), times = nrow(VC_matrix)),
  y = rep(1:nrow(VC_matrix), each = nrow(VC_matrix))
)

ggplot(VC_matrix_df, aes(x=x,y=y,fill=log(value))) +
    geom_raster() +
    scale_y_reverse() +
    guides(fill = 'none') +
