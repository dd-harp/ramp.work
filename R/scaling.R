...
#' @param N an integer
#' @param rbr the relative biting rate
...
xde_scaling_eir = function(model, N=25, rbr=1){
  ...
  for(i in 1:N){
    ...
    eir_t = tail(XH$eir, 365)/rbr; eir[i] = mean(eir_t)
...
#' @param model a list that defines an `ramp.xds` model generated by `xde_setup_human()`
...
xde_scaling_Z = function(model, N=25){
  ...
  model <- ramp.xds::make_indices(model)
  y0 = ramp.xds::get_inits(model)

  # Get the
  F_Z_base = model$MYZpar$Zf
  wts_f = as.vector(model$BFpar$searchWts[[1]][[1]])
  H = F_H(get_inits(model), model, 1)
