...
#' @export
fit_EIR_phase_sin_season <- function(data, times, model, Fp=NULL){
  ...
  for(i in 1:length(ph)){
    ...
    ss[i] <- sse_EIR_season(data, times, model)
  }
  ix = which.min(ss)
  ff <- ss[ix]
  best <- ph[ix]
  ...
  for(i in 1:length(ph1)){
    ...
    ss1[i] <- sse_EIR_season(data, times, model)
  }
  ix1 <- which.min(ss1)
  ff1 <- ss1[ix1]
  best1 <- ph1[ix1]
  ...
  for(i in 1:length(ph2)){
    ...
    ss2[i] <- sse_EIR_season(data, times, model)
  }
...
#' @return a list with the mean peak and the values
#' @export
sse_EIR_season <- function(data, times, model){
  model <- xds_solve_cohort(model, times=times)
...
#' @return an `xds` object
#' @export
fit_EIR_amplitude_sin_season <- function(data, times, model, Fpar=NULL){
  ...
  F_eval = function(X, data, times, model){
    ...
    sse_EIR_season(data, times, model)
  }
...
#' @return an `xds` object
#' @export
fit_EIR_sin_season <- function(data, times, model, Fpar=NULL){
  ...
  model$EIRpar$season_par$bottom = pars_seas[1]^2
  model$EIRpar$season_par$pw = Fpw(pars_seas[2])
  model$EIRpar$season_par$phase = pars_seas[3]
  model <- xds_solve_cohort(model, times=times)
  return(model)
}
