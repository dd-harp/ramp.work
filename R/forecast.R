...
#' @export
forecast_spline= function(spline_par, N=5){
  # Time points
  tt <- spline_par$tt
...
#' @param N the number of years ahead to forecast
#' @export
forecast_spline_EIR = function(model, N=5){
  model$EIRpar$trend_par <- forecast_spline(model$EIRpar$trend_par, N)
...
#' @export
forecast_spline_Lambda = function(model, N=5, s=1){
  model$Lpar[[s]]$trend_par <- forecast_spline(model$Lpar[[s]]$trend_par, N)
