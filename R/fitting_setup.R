...
setup_trend_par.Lambda= function(xds_obj){
  ...
  xds_obj$Lpar[[1]]$trend_par <- Tp
  xds_obj$Lpar[[1]]$F_trend <- make_function(Tp)
  return(xds_obj)
}
...
setup_trend_par.eir = function(xds_obj){
  Tp <- with(xds_obj$fitting, makepar_F_spline(tt, yy, X=2))
  xds_obj$EIRpar$trend_par <- Tp
  xds_obj$EIRpar$F_trend <- make_function(Tp)
  return(xds_obj)
}
...
check_season_par.Lambda= function(xds_obj){
  Sp <- setup_season(xds_obj$Lpar[[1]]$season_par)
  xds_obj$Lpar[[1]]$season_par <- Sp
  xds_obj$Lpar[[1]]$F_season <- make_function(Sp)
  return(xds_obj)
}
...
check_season_par.eir = function(xds_obj){
  Sp <- setup_season(xds_obj$EIRpar$season_par)
  xds_obj$EIRpar$season_par <- Sp
  xds_obj$EIRpar$F_season <- make_function(Sp)
  return(xds_obj)
}
...
scaling_init_ty.Lambda = function(xds_obj){
  ...
  xds_obj$Lpar[[1]]$Lambda <- L

#  xds_obj <- fit_mean_forcing(xds_obj)
#  L <- xds_obj$Lpar[[1]]$Lambda
