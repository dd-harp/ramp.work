...
+ `xds_scaling` is a wrapper for `xde_scaling` and `dts_scaling` 

+ `xde_scaling` and `dts_scaling` dispatch on `xds_obj$forced_by` 

The functions create a mesh over mean forcing and for each value, they compute stable orbits and the annual average values of the metrics.  These are attached as `xds_obj$scaling.`
...
xde_scaling(sis_full, 1.1, 30) -> sis_full
```
