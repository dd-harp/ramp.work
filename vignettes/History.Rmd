...
In malaria, the immune status of the population depends on the history of exposure. Here, we develop a method to estimate the history of exposure from a time series describing the *Pf*PR in a population over time. The algorithm is called `pr2history` and it can be run in two ways: 
